...
void GetModelParams(int Date, float ut, const char *Model, int *iopt, double *parmod, double *tilt, double *Vx, double *Vy, double *Vz) {
	...
	//		return;
		/* Then T96 is Pdyn, Dst (SymH in this case), By, Bz */
			...
			parmod[1] = (double) InterpParam(TSData.SymH,Date,ut);
			parmod[2] = (double) InterpParam(TSData.By,Date,ut);
	...
	//		return;
		/* Next T01 which uses: Pdyn, Dst, By, Bz, G1, G2*/
			...
			parmod[2] = (double) InterpParam(TSData.By,Date,ut);
			parmod[3] = (double) InterpParam(TSData.Bz,Date,ut);
			parmod[4] = (double) InterpParam(TSData.G1,Date,ut);
			parmod[5] = (double) InterpParam(TSData.G2,Date,ut);
	//		return;
		/*TS05: Pdyn, Dst, By, Bz, W1, W2, W3, W4, W5, W6*/
			...
			parmod[2] = (double) InterpParam(TSData.By,Date,ut);
			parmod[3] = (double) InterpParam(TSData.Bz,Date,ut);
			parmod[4] = (double) InterpParam(TSData.W1,Date,ut);
			parmod[5] = (double) InterpParam(TSData.W2,Date,ut);		
			parmod[6] = (double) InterpParam(TSData.W3,Date,ut);
			parmod[7] = (double) InterpParam(TSData.W4,Date,ut);
			parmod[8] = (double) InterpParam(TSData.W5,Date,ut);
			parmod[9] = (double) InterpParam(TSData.W6,Date,ut);	
	//		return;
		} 
	}
	if (strchr(Model,'c') != NULL) {
		...
		for (i=0;i<10;i++) {
			if (!isnan(CustP.parmod[i])) {
				parmod[i] = (double) CustP.parmod[i];
			}
		}
	}
