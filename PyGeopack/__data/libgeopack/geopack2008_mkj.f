...
C      WRITE(*,*) XGSW,YGSW,ZGSW,XGEO,YGEO,ZGEO
      RHO2=XGEO**2+YGEO**2
      R=SQRT(RHO2+ZGEO**2)
      C=ZGEO/R
      RHO=SQRT(RHO2)
      ...
      PP=1./R
      P=PP
C	WRITE(*,*) RHO2,R,C,RHO,S,CF,SF,PP,P
C
...
211   HE=BR*S+BT*C
      ...
      HYGEO=HE*SF+BF*CF
      HZGEO=BR*C-BT*S
c		WRITE(*,*) Br,BT,Bf,C,S,CF, SF
      CALL GEOGSW_08 (HXGEO,HYGEO,HZGEO,HXGSW,HYGSW,HZGSW,1)
c      WRITE(*,*) HE,HXGEO,HYGEO,HZGEO,HXGSW,HYGSW,HZGSW
C
      RETURN
      END
C
...
C
      ...
      COSD=SQRT(1.-SIND**2)
      SC=SIND/COSD
C      WRITE(*,*) "SUN: ",COS(OBLIQ),SOB*SC
C      WRITE(*,*) "SUN: ",COS(OBLIQ)/SOB*SC,-COS(SLP)/COSD
      SDEC=ATAN(SC)
      SRASN=3.141592654-ATAN2(COS(OBLIQ)/SOB*SC,-COS(SLP)/COSD)
C      WRITE(*,*) "SUN: ",GST,SLONG,SRASN,SDEC
      RETURN
      END
C
C================================================================================
...
C
      DATA ISW /1/ !Line changed by RCF to suppress warning about GSM/GSW system
c
...
C
      DATA G15/0.0,-29442.0,-1501.0, -2445.1,  3012.9,  1676.7,  1350.7,
...
C
     ...
     *        -6.8,    -6.9,    7.8,     1.0,      -4,     8.4,     0.0,
     *        -0.4,     4.6,    4.4,    -7.9,    -0.6,    -4.2,    -2.8,
     *         3.2,    -1.2,   -8.7,     0.0,    -0.1,     2.0,    -0.7,
...
C
cC
c      DATA DG10/0.0,  11.4,    16.7,   -11.3,    -3.9,     2.7,     1.3,
c     *         -3.9,  -2.9,    -8.1,    -1.4,     2.0,    -8.9,     4.4,
c     *         -2.3,  -0.5,     0.5,    -1.5,    -0.7,     1.3,     1.4,
c     *         -0.3,  -0.3,    -0.3,     1.9,    -1.6,    -0.2,     1.8,
c     *          0.2,  -0.1,    -0.6,     1.4,     0.3,     0.1,    -0.8,
c     *          0.4,  -0.1,     0.1,    -0.5,     0.3,    -0.3,     0.3,
c     *          0.2,  -0.5,     0.2/
cC
c      DATA DH10/0.0,   0.0,   -28.8,     0.0,   -23.0,   -12.9,     0.0,
c     *          8.6,  -2.9,    -2.1,     0.0,     0.4,     3.2,     3.6,
c     *         -0.8,   0.0,     0.5,     1.5,     0.9,     3.7,    -0.6,
c     *          0.0,  -0.1,    -2.1,    -0.4,    -0.5,     0.8,     0.5,
c     *          0.0,   0.6,     0.3,    -0.2,    -0.1,    -0.8,    -0.3,
c     *          0.2,   0.0,     0.0,     0.2,     0.5,     0.4,     0.1,
c     *         -0.1,   0.4,     0.4/
cC
C
...
C      ####Removed the warnings here, they become annoying when doing lots
C      of calculations
      IF(IY.LT.1965) THEN
       IY=1965
C       WRITE (*,10) IYEAR,IY
...
C
      ...
      IF (IY.LT.2015) GOTO 140         !INTERPOLATE BETWEEN 2005 - 2010
C      The code below has been updated to extrapolate from 2015 instead of 2010
...
C
300   S=1.
C	  WRITE(*,*) F1,F2,FLOAT(IY),FLOAT(IDAY-1)/365.25,2005
...
C
      ...
      CTSL=CT0*SL0
      CTCL=CT0*CL0
C      WRITE(*,*) SQ,SQQ,SQR,SL0,CL0,ST0,CT0,STCL,STSL,CTSL,CTCL
C
...
C
     ...
     * -0.5+FLOAT(IHOUR*3600+MIN*60+ISEC)/86400.
      ...
      DZ2=-SIN(OBLIQ)
      DZ3=COS(OBLIQ)
C       WRITE(*,*) IY,IDAY,IHOUR,MIN,ISEC,DJ,T,Obliq,DZ1,DZ2,DZ3
...
C     WRITE(*,*) X,Y,Z,DS3,BXGSW,BYGSW,BZGSW,HXGSW,HYGSW,HZGSW,B
      R1=BX*B
      R2=BY*B
      R3=BZ*B
      RETURN
...
C
  2   X=X+.5*(R11+4.*R41+R51)
      Y=Y+.5*(R12+4.*R42+R52)
      Z=Z+.5*(R13+4.*R43+R53)
C		WRITE(*,*) X, Y, Z, DS3, R11, R12, R13, R21, R22, R23, R31, R32,
C     *		 R33, R41, R42, R43, R51, R52, R53, ErrCur
...
c
  6   R1=(R0-R)/(RR-R)
      ...
      Z=Z-(Z-ZR)*R1
      GOTO 8
C  7   WRITE (*,10)  this is a highly annoying warning 
  7   L=LMAX
  8   XF=X
      YF=Y
      ZF=Z
